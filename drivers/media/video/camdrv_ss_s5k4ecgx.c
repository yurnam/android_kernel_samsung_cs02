...
#else
#ifdef CONFIG_FLASH_RT8547
static int camdrv_ss_s5k4ecgx_RT8547_flash_control(struct v4l2_subdev *sd, int control_mode);
#else
...
static int camdrv_ss_s5k4ecgx_set_ae_stable_status(struct v4l2_subdev *sd, unsigned char value);//Nikhil
...
#else
#ifdef CONFIG_FLASH_RT8547
#define camdrv_ss_s5k4ecgx_flash_control(a,b)  camdrv_ss_s5k4ecgx_RT8547_flash_control(a,b)
#else
...
#if defined(CONFIG_MACH_HAWAII_SS_LOGAN_REV02)|| defined(CONFIG_MACH_HAWAII_SS_LOGAN_REV03) \
|| defined(CONFIG_MACH_HAWAII_SS_LOGANDS_REV00)|| defined(CONFIG_MACH_HAWAII_SS_LOGANDS_REV01) \
|| defined(CONFIG_MACH_HAWAII_SS_HEAT_REV00)|| defined(CONFIG_MACH_JAVA_SS_EVAL_REV00) \
...
int camdrv_ss_s5k4ecgx_set_preview_start(struct v4l2_subdev *sd)
{
	...
	else if(state->mode_switch == CAMCORDER_PREVIEW_TO_CAMERA_PREVIEW)
	{
		...
		if (ARRAY_SIZE(s5k4ecgx_fps_auto_regs) != (int)NULL)
		{
...
/*NIKHIL*/
static int camdrv_ss_s5k4ecgx_set_ae_stable_status(struct v4l2_subdev *sd, unsigned char value)
{ 
 struct i2c_client *client = v4l2_get_subdevdata(sd);
    int err = 0;
    ...
    err += camdrv_ss_i2c_write_2_bytes(client, (unsigned char)0x0F12, value);
...
//        	err = camdrv_ss_i2c_set_config_register(client,s5k4ecgx_ae_unlock_regs, ARRAY_SIZE(s5k4ecgx_ae_unlock_regs), "ae_unlock_regs");
	...
	if (err < 0) {
	   ...
	   first_af_status = false;
		...
		if (state->camera_flash_fire == 1) {
			camdrv_ss_s5k4ecgx_set_ae_stable_status(sd, 0x0002);
			if (ARRAY_SIZE(s5k4ecgx_Pre_Flash_End_EVT1) == (int)NULL) {
...
static int camdrv_ss_s5k4ecgx_set_touch_focus_area(struct v4l2_subdev *sd, enum v4l2_touch_af touch_af, v4l2_touch_area *touch_area)
{
   ...
   // unsigned short FirstWinEndX, FirstWinEndY, ConvFirstWinStartX, ConvFirstWinStartY;
    unsigned short ConvFirstWinStartX, ConvFirstWinStartY;
    //unsigned short SecondWinEndX, SecondWinEndY, 
    unsigned short ConvSecondWinStartX, ConvSecondWinStartY;
   ...
   // struct v4l2_control ctrl = {0};
	...
	if(touch_af == TOUCH_AF_STOP)	{ /* AF stop */
	 ...
	 ly = preview_height - touch_area->leftTopY;
        ...
        SecondWinStartY = ly - ( (AF_INNER_WINDOW_HEIGHT- touch_area->rightBottomY) / 2 );
        ...
        FirstWinStartX = lx - ( (AF_OUTER_WINDOW_WIDTH - touch_area->rightBottomX) / 2 );
        if (FirstWinStartX < 0)
			FirstWinStartX = 0;
        if (FirstWinStartX+AF_OUTER_WINDOW_WIDTH > preview_width)
            FirstWinStartX = preview_width-AF_OUTER_WINDOW_WIDTH-1;
        
        FirstWinStartY = ly - ( (AF_OUTER_WINDOW_HEIGHT- touch_area->rightBottomY) / 2 );
        if (FirstWinStartY < 0)
			FirstWinStartY = 0;
...
//		err = camdrv_ss_i2c_set_config_register(client,s5k4ecgx_awb_unlock_regs, ARRAY_SIZE(s5k4ecgx_awb_unlock_regs), "awb_unlock_regs");
	...
	else if((int)touch_af == (int)AUTO_FOCUS_1ST_CANCEL)
	{
		if (state->af_mode == FOCUS_MODE_AUTO) {
	    ...
	    else if((int)touch_af == (int)AUTO_FOCUS_2ND_CANCEL )
	   {
                        if (state->af_mode == FOCUS_MODE_AUTO) {
...
CAM_INFO_PRINTK( "%s %d :  E\n",  __func__, control_mode);
	...
	case FLASH_CONTROL_MAX_LEVEL:
		{	
			CAM_INFO_PRINTK( "SWSW: FLASH_CONTROL_MAX_LEVEL");
        ...
        case FLASH_CONTROL_HIGH_LEVEL:
		{	
			CAM_INFO_PRINTK( "SWSW: FLASH_CONTROL_HIGH_LEVEL");
		//	int reg;
		//	unsigned char reg_val;
			// always set ctlen low & flashen low first to ensure the state is really off
			gpio_set_value(GPIO_CAM_FLASH_EN, 0);
        ...
        case FLASH_CONTROL_MIDDLE_LEVEL:
			...
			//int reg;
			//unsigned char reg_val;
			CAM_INFO_PRINTK( "SWSW: FLASH_CONTROL_MIDDLE_LEVEL");
			// always set ctlen low & flashen low first to ensure the state is really off
			gpio_set_value(GPIO_CAM_FLASH_EN, 0);
			gpio_set_value(GPIO_CAM_FLASH_SET, 0);
		...
		//	int reg;
		//	unsigned char reg_val;
			CAM_INFO_PRINTK( "SWSW: FLASH_CONTROL_LOW_LEVEL");
			// always set ctlen low & flashen low first to ensure the state is really off
			gpio_set_value(GPIO_CAM_FLASH_EN, 0);
			gpio_set_value(GPIO_CAM_FLASH_SET, 0);
        ...
        case FLASH_CONTROL_OFF:
		{
			CAM_INFO_PRINTK( "SWSW: FLASH_CONTROL_OFF_LEVEL");
			gpio_set_value(GPIO_CAM_FLASH_EN, 0);
			gpio_set_value(GPIO_CAM_FLASH_SET, 0);			
			break;
        	}
...
/*optional*/
	...
	#else
	#ifdef CONFIG_FLASH_RT8547
	sensor->flash_control    	   = camdrv_ss_s5k4ecgx_RT8547_flash_control;
	#else
